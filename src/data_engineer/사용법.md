...
from transform_module import train_transforms
from transform_module import test_transforms
...
train_transforms, test_transforms = transform_module.v1_transform       # 선명도 100, 회전 360 적용
#train_transforms, test_transforms = transform_module.v2_transform      # 회전 360 적용
#train_transforms, test_transforms = transform_module.v3_transform      # 선명도 100 적용

#train_transforms, test_transforms = transform_module.n_v1_transform    # 선명도 100, 회전 360 적용
#train_transforms, test_transforms = transform_module.n_v2_transform    # 회전 360 적용
#train_transforms, test_transforms = transform_module.n_v3_transform    # 선명도 100 적용

tr_train_dataset = FasterRCNNDataset(image_dir, anntation_dir, class_dict, transforms=train_transforms, img_path_list_mode=True, img_path_list=train_imgs_path)   #      transform 적용 dataset 만들기
tr_valid_dataset = FasterRCNNDataset(image_dir, anntation_dir, class_dict, transforms=test_transforms, img_path_list_mode=True, img_path_list=val_imgs_path)     #      transform 적용 test dataset 만들기

print(len(train_dataset), len(valid_dataset))

train_dataloader = Faster_RCNN_dataloader.full_train_build_dataloaders(tr_train_dataset,4)
valid_dataloader = Faster_RCNN_dataloader.full_train_build_dataloaders(tr_valid_dataset,4)
"""
...
yolo_data_split.py
yolo_img_converter.py
class_mapping.py

yolo_img_converter.py 함수 목록:
apply_v2_compose_and_save(transform, image_dir, master_dir, output_dir_name="converted img", load_exts = ("*.jpg", "*.png", "*.jpeg")):    return save_dir
...
a_list = None
a_list = get_file_name_YOLO(anntation_dir,image_dir)                                        #   <-- a_list(valid 데이터 이름 리스트를 가져옴)
...
#image_dir = apply_v2_compose_and_save(transforms, image_dir, master_dir, output_dir_name="converted img")        # RandomAdjustSharpness 적용 시 활성화
