#======================================================================================
# YOLO 실험 3번: 데이터 준비 및 증강 전용 스크립트
# - 원본 데이터로부터 YOLO annotation 생성
# - train/val 분할 후 experiment3 폴더에 복사
# - RandomRotation(360) 변환으로 선택된 파일들에 대해 증강 데이터 생성
# - 이 스크립트는 한 번만 실행하면 됨
#======================================================================================
import os
import sys
...
def prepare_experiment3_data():
    """YOLO 실험 3번을 위한 데이터 준비 (원본 + 증강 데이터)"""
    print("\n" + "="*80)
    print("[YOLO 실험 3번] 데이터 준비 및 증강 시작")
    print("="*80 + "\n")
    ...
    yolo_annt_dir = make_YOLO_annotation(image_dir, annotation_dir, class_dict, "YOLO_annotation_exp3")
    ...
    # experiment3 폴더 생성
    experiment3_dir = os.path.join(ROOT_DIR, 'data', 'yolo_dataset', 'experiment3')
    
    print("\n" + "="*80)
    print(f"▶ [3단계] experiment3 폴더 생성 및 원본 데이터 복사")
    print(f" - experiment3 경로: {os.path.abspath(experiment3_dir)}")
    print("="*80 + "\n")
    
    # experiment3 폴더가 이미 있으면 삭제
    if os.path.exists(experiment3_dir):
        print(f"기존 experiment3 폴더 삭제 중...")
        shutil.rmtree(experiment3_dir)
    
    # 임시 폴더의 모든 데이터를 experiment3로 복사
    print("원본 train/val 데이터를 experiment3 폴더로 복사 중...")
    shutil.copytree(temp_split_dir, experiment3_dir)
    print(f"✓ 복사 완료: {temp_split_dir} → {experiment3_dir}")
    ...
    data_yaml_path = os.path.join(model_yolo_dir, 'data_experiment3.yaml')
    ...
    print(f"  - Path: {experiment3_dir}\n")
    
    # 증강 데이터 생성
    print("="*80)
    print(f"▶ [5단계] 증강 데이터 생성 (RandomRotation)")
    print("="*80 + "\n")
    
    img_train_dir = os.path.join(experiment3_dir, "images", "train")
    label_train_dir = os.path.join(experiment3_dir, "labels", "train")
    
    # RandomRotation transform 정의
    transforms = v2.Compose([
        v2.RandomRotation(360)
    ])
    
    # 데이터 실험.md의 파일 리스트 정의 (228-344 라인)
    ...
    print(f" - experiment3 폴더: {experiment3_dir}")
    print(f" - data.yaml: {data_yaml_path}")
    print("="*80 + "\n")
    
    return experiment3_dir

if __name__ == "__main__":
    result = prepare_experiment3_data()
    if result:
        print("\n 성공! 이제 yolo_train_experiment3.py를 실행하여 학습을 시작하세요.")
        print("   명령어: python src/model/yolo/yolo_train_experiment3.py")
    else:
        print("\n 데이터 준비 실패")

### 실행 명령어: python src/data_engineer/prepare_experiment3_data.py ###
